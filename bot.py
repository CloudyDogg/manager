    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Ошибка при отправке инструкций с изображениями: {e}")
...
async def select_chat_callback(client, callback_query):
    ...
    try:
        ...
        if success:
            ...
            user_details += f"📋 <b>Основная информация:</b>\n"
            user_details += f"ID: <code>{user_id}</code>\n"
            ...
            user_details += f"📱 <b>Дополнительно:</b>\n"
            user_details += f"Язык: {user_info.language_code or 'неизвестно'}\n"
            user_details += f"Премиум: {'✅' if user_info.is_premium else '❌'}\n"
            ...
            if "приватности" in message:
                # В случае настроек приватности, ссылка и инструкции уже отправлены в add_user_to_chat
                ...
                # Отправляем администраторам информацию о пользователе, который не смог быть добавлен
                user_info = await client.get_users(user_id)
                
                user_details = f"⚠️ Новая заявка (требуется ссылка-приглашение):\n\n"
                user_details += f"📋 <b>Информация о пользователе:</b>\n"
                user_details += f"ID: <code>{user_id}</code>\n"
                user_details += f"Имя: {user_info.first_name} {user_info.last_name or ''}\n"
                user_details += f"Username: @{user_info.username or 'отсутствует'}\n\n"
                user_details += f"🔒 <b>Причина:</b> Ограничения приватности пользователя\n"
                user_details += f"📧 <b>Действие:</b> Отправлена ссылка-приглашение и инструкции\n"
                user_details += f"📅 <b>Дата:</b> {datetime.now().strftime('%d.%m.%Y %H:%M')}\n"
